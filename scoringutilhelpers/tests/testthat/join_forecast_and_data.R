...
test_that("join_forecast_and_data correctly joins forecast and actual data", {
  ...
  actual_data <-
    readr::read_tsv("scoringutilhelpers/assets/example_truth_data.tsv")
  scoringdataset <- join_forecast_and_data(forecast_source, truth_data_file,
    join_key = join_by(area, target_end_date == date)
  ) |> collect()
  expect_true(all(c(
    "area", "reference_date", "target_end_date", "truth_data",
