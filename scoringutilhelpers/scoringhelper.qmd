...
The `example_prediction` function generates log-normal "forecasts" across a user selected number of areas and dates with 1 and 2 week lookaheads.
...
The `example_truth_data` function generates "truth_data" in a similar way, and also serialises to `/assets`.
...
1. Ingesting forecasts from the `forecast_source` directory and truth_data from `truth_data_file`. The forecasts are treated as an Arrow dataset and loaded with `arrow::open_dataset`. The truth_data is assumed to be a `tsv` file.
