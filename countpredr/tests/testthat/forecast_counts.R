...
test_that("forecast_counts handles different forecast horizons", {
  result_1_week <- forecast_counts(test_data,
    count_col = "other_ed_visits", date_col = "date", h = "1 week"
  )
  result_4_weeks <- forecast_counts(test_data,
    count_col = "other_ed_visits", date_col = "date", h = "4 weeks"
  )
...
test_that("forecast_counts handles missing values gracefully", {
  ...
  result <- forecast_counts(test_data_with_na,
    count_col = "other_ed_visits", date_col = "date", h = "3 weeks"
  )
...
test_that("forecast_counts handles different count columns", {
  ...
  result <- forecast_counts(test_data_renamed,
    count_col = "new_count", date_col = "date", h = "3 weeks"
  )
...
test_that("forecast_counts samples the correct number of times", {
  result <- forecast_counts(test_data,
    count_col = "other_ed_visits",
    date_col = "date", h = "3 weeks", times = 2000
  )

  # Check if the number of samples is correct
  # divide by 3 because of the 3 models used
  expected_samples <- 2000
  actual_samples <- nrow(result$predictive_samples) /
    (3 * length(unique(result$predictive_samples$date)))
