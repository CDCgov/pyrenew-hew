...
dur_shed_rv = DistributionalVariable(
    "dur_shed", dist.TruncatedNormal(dur_shed_mean, dur_shed_sd, low=0)
)
```
...
eta_i_first_obs_rv = DistributionalVariable(
    "eta_i_first_obs", dist.Normal(0,1)
)

eta_initial_exp_growth_rate_rv = DistributionalVariable(
    "eta_initial_exp_growth_rate", dist.Normal(0,1)
)
...
#     "mean_initial_exp_growth_rate", dist.Normal(
#         mean_initial_exp_growth_rate_prior_mean, mean_initial_exp_growth_rate_prior_sd
#     )
# )
...
```{python}
latent_hospital_admissions = test_model_sample[0]
observed_hospital_admissions =test_model_sample[1]
ww_pred = test_model_sample[2]
exp_state_ww_conc=test_model_sample[3]
state_rt = test_model_sample[4]
r_site_t = test_model_sample[5]
rtu_site = test_model_sample[6]
state_inf_per_capita = test_model_sample[7]
```
...
```{python}
state_inf_per_capita*state_pop
```
...
my_model.run(
    ...
    mcmc_args=dict(num_chains=2, progress_bar=False),
)
```
...
```{python}
idata.posterior.r_site_t
```

```{python}
idata.posterior.log_i0_site
```
