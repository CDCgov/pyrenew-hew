...
```
```{python}
n_runs = 10  # Number of times to run the sampling
base_key = jax.random.PRNGKey(242)
samples = []
for i in range(n_runs):
    key = jax.random.split(base_key, n_runs)[i]
    with numpyro.handlers.seed(rng_seed=key):
        sample = my_model.sample(n_datapoints=50)
        samples.append(sample)

# Only samples returned is I0
print(samples)
```
