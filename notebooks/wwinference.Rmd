...
outputs against data. This can be useful to get a sense of if your model is
...
file_copy(
  path = fit$raw_fit_obj$data_file(),
  new_path = path(fit_dir, "stan_data", ext = "json"),
  overwrite = TRUE
)
file_copy(
  path = fit_hosp_only$raw_fit_obj$data_file(),
  new_path = path(fit_hosp_only_dir, "stan_data", ext = "json"),
  overwrite = TRUE
)

fit$raw_fit_obj$save_output_files(fit_dir)
fit_hosp_only$raw_fit_obj$save_output_files(fit_hosp_only_dir)
```
