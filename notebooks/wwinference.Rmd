...
```{r wastewater-time-series-fig,  out.width='100%'}
ggplot(ww_data_preprocessed) +
  ...
  theme_bw()
```
```{r hosp-time-series-fig,  out.width='100%'}
...
We can generate this using the `ww_fit` object as demonstrated below:
...
params$sigma_rt_prior <- 0.0001 # This model contains a single "site," so we
# decrease the prior variance to make the "site" Rt nearly the same as the
# "global" Rt.
...
base_dir <- path("notebooks", "data")
fit_dir <- path(base_dir, "fit")
fit_hosp_only_dir <- path(base_dir, "fit_hosp_only")
dir_create(fit_dir)
dir_create(fit_hosp_only_dir)


file_copy(
  path = ww_fit$fit$result$data_file(),
  new_path = path(fit_dir, "stan_data", ext = "json"),
  overwrite = TRUE
)
file_copy(
  path = fit_hosp_only$fit$result$data_file(),
  new_path = path(fit_hosp_only_dir, "stan_data", ext = "json"),
  overwrite = TRUE
)

ww_fit$fit$result$save_output_files(fit_dir)
fit_hosp_only$fit$result$save_output_files(fit_hosp_only_dir)
```
