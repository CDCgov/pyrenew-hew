...
def create_epiautogp_input(
    csv_path: str,
    report_date: str,
    output_path: str,
    normal_mean: float = 0.1,
    normal_std: float = 0.03,
    n_samples: int = 100,
):
    """
    ...
    nowcast_samples = [
        float(last_report * multiplier) for multiplier in lognormal_multipliers
    ]

    print(f"Generated {n_samples} nowcast samples")
    print(
        f"Multiplier range: {min(lognormal_multipliers):.4f} to {max(lognormal_multipliers):.4f}"
    )
    print(
        f"Nowcast sample range: {min(nowcast_samples):.1f} to {max(nowcast_samples):.1f}"
    )
...
def main():
    ...
    result = create_epiautogp_input(
        csv_path, report_date, output_path, normal_mean, normal_std, n_samples
    )
    ...
    if len(result["nowcast_reports"]) > 0:
        ...
        all_values = [
            vec[0] for vec in result["nowcast_reports"]
        ]  # Extract first (and only) value from each vector
        print(
            f"Nowcast samples: {len(result['nowcast_reports'])} realizations, each with {len(first_realization)} values, range {min(all_values):.1f}-{max(all_values):.1f}"
        )
