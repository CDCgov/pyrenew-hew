...
@testset "create_forecast_df Function Tests" begin
    @testset "Basic Functionality" begin
        ...
        result_df = create_forecast_df((forecast_dates=forecast_dates, forecasts=forecasts), output_type)
    ...
    @testset "Single Date and Single Quantile" begin
        ...
        result_df = create_forecast_df((forecast_dates=single_date, forecasts=single_forecasts), output_type)
    ...
    @testset "Multiple Quantiles with Known Data" begin
        ...
        result_df = create_forecast_df((forecast_dates=forecast_dates, forecasts=known_data), output_type)
    ...
    @testset "Default Quantile Levels" begin
        ...
        result_df = create_forecast_df((forecast_dates=forecast_dates, forecasts=forecasts), default_output)
    ...
    @testset "Edge Cases and Error Conditions" begin
        ...
        empty_result = create_forecast_df((forecast_dates=empty_dates, forecasts=empty_forecasts), output_type)
        ...
        empty_quantile_result = create_forecast_df((forecast_dates=forecast_dates, forecasts=forecasts), empty_quantiles)
        ...
        single_result = create_forecast_df((forecast_dates=single_date, forecasts=single_sample), output_type)
    ...
    @testset "Data Type and Precision Tests" begin
        ...
        result_df = create_forecast_df((forecast_dates=forecast_dates, forecasts=integer_data), output_type)
        @test result_df.value[1] == 3.0  # Median of 1,2,3,4,5

        # Test with very small numbers
        small_data = reshape([1e-10, 2e-10, 3e-10], 1, 3)
        small_result = create_forecast_df((forecast_dates=forecast_dates, forecasts=small_data), output_type)
        @test small_result.value[1] ≈ 2e-10

        # Test with very large numbers
        large_data = reshape([1e10, 2e10, 3e10], 1, 3)
        large_result = create_forecast_df((forecast_dates=forecast_dates, forecasts=large_data), output_type)
        @test large_result.value[1] ≈ 2e10

        # Test precision preservation
        precise_data = reshape([1.123456789, 1.123456790, 1.123456791], 1, 3)
        precise_result = create_forecast_df((forecast_dates=forecast_dates, forecasts=precise_data), output_type)
    ...
    @testset "Date Handling and String Conversion" begin
        ...
        result_df = create_forecast_df((forecast_dates=diverse_dates, forecasts=forecasts), output_type)
    ...
    @testset "Statistical Properties" begin
        ...
        output_type = QuantileOutput(quantile_levels = [0.5])
        result_df = create_forecast_df((forecast_dates=forecast_dates, forecasts=normal_data), output_type)
        ...
        extreme_result = create_forecast_df((forecast_dates=forecast_dates, forecasts=normal_data), extreme_output)
...
@testset "Integration Tests" begin
    @testset "Realistic Forecasting Scenario" begin
        ...
        result_df = create_forecast_df((forecast_dates=forecast_dates, forecasts=forecasts), realistic_output)
    ...
    @testset "Cross-validation with Manual Calculations" begin
        ...
        output_type = QuantileOutput(quantile_levels = [0.0, 0.5, 1.0])
        result_df = create_forecast_df((forecast_dates=forecast_dates, forecasts=simple_forecasts), output_type)
