...
def get_initialization(stan_data, stdev, rng_key):
    i_first_obs_est = np.mean(stan_data["hosp"][:7]) / stan_data["p_hosp_prior_mean"]
    logit_i_frac_est = jax.scipy.special.logit(i_first_obs_est / stan_data["state_pop"])
    ...
    init_vals = {
        ...
        "sigma_initial_exp_growth_rate": jnp.abs(dist.Normal(0, stdev).sample(rng_key)),
        ...
        "log10_g": dist.Normal(stan_data["log10_g_prior_mean"], 0.5).sample(rng_key),
