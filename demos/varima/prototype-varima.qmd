...
us_data_mjr_peak <- us_data |> filter(time_value < wk_bf_mj_peak)

fits_mjr_peak <- model_configs_covid |>
    map(function(x) {
    us_data_mjr_peak |> model(!!x$name := x$model) } )

forecasts_mjr_peak <- fits_mjr_peak |>
    map(\(x) forecast(x, h = fc_str))

accuracy_results <- forecasts_mjr_peak |>
    map(\(x) accuracy(x, us_data, measures = list(crps_score = CRPS))) |>
    bind_rows() |>
    mutate(.response = ifelse(is.na(.response), "log_nhsn_covid", .response)) |>
    filter(.response == "log_nhsn_covid") |>
...
```{r}
summary_forecasts <- fits_mjr_peak |>
    map(
        function(x){
            x |>
            generate(times = 2000, h = fc_str) |>
            as_tibble() |>
