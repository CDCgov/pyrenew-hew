...
plotting_rt_subpop <-
  bind_rows(
    deframe(pyrenew_tidy_draws)$posterior_predictive |>
      gather_draws(rt_subpop[time,group]) |>
      median_qi(.width = 0.8) |>
      mutate(model = "pyrenew"),
    stan_tidy_draws |>
      gather_draws(r_subpop_t[group,time]) |>
      mutate(.variable = case_when(
        .variable == "r_subpop_t" ~ "rt_subpop",
        TRUE ~ .variable
      )) |>
...
plotting_rt_subpop |>
  ...
  mutate(group = if_else(model == "pyrenew", group + 1, group)) |>  #adjust for index python starting from 1
  ggplot(aes(time, .value, color = model)) +
  facet_wrap(vars(group)) +
  geom_lineribbon(aes(ymin = .lower, ymax = .upper, fill = model), alpha = 0.5) +
  scale_fill_brewer(
    name = "Credible Interval Width",
    labels = ~ percent(as.numeric(0.8))
  ) +
  cowplot::theme_cowplot() +
  ggtitle("Vignette Data Model Comparison") +
